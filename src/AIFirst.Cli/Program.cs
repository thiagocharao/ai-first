...
public static class Program
{
    ...
    private static async Task<int> PullToolsAsync(string[] args)
    {
        if (args.Length < 1)
        {
            ...
            Console.Error.WriteLine("Example: aifirst pull-tools npx @modelcontextprotocol/server-filesystem /tmp");
            return 1;
        }
    ...
    private static int GenerateCode(string[] args)
    {
        ...
        if (!File.Exists(manifestPath))
        {
            ...
            Console.Error.WriteLine();
            Console.Error.WriteLine("Usage: aifirst gen [manifest] [options]");
            Console.Error.WriteLine("  --namespace, -n <name>   Namespace for generated code (default: Generated)");
            Console.Error.WriteLine("  --output, -o <path>      Output file path (default: Tools.g.cs)");
            return 1;
        }
